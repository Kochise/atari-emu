...
{       CELLPTR cellptr;
        ...
        cellptr = (CELLPTR)(malloc(valuecellsize));
...
{       writef(strlen(MSGMEMORY) + 2, 1, 6, "%6ld", memleft);
} /* printfreemem */
...
{       putchar('\a');      /* Beeps the speaker */
        writef(1, 25, 79, "%s  %s", s, MSGKEYPRESS);
...
{       int total = 80, col = 0;
        ...
        if(total >= LEFTMARGIN)
                col++;
        memmove(colstart, &colstart[SCREENCOLS - col + 1], col - 1);
...
{       while((col < MAXCOLS) && (format[col][row] >= OVERWRITE) &&
              (cell[col][row] == NULL))
        {       format[col][row] &= ~OVERWRITE;
                if(display && (col >= leftcol) && (col <= rightcol))
                        displcell(col, row, NOHIGHLIGHT, NOUPDATE);
...
{       while((cell[col][row] == NULL) && (col-- > 0))
                ;
        if((col >= 0) && (cell[col][row]->attrib == TEXT))
...
{       char s[81];
        ...
        {       if(formatting)
                {       sprintf(vstring, "%1.*lf", fvalue & 15, cellptr->v.value);
                        if(fvalue & COMMAS)
                        {       pos = (int) strcspn(vstring, ".");
                                while(pos > 3)
                                {       pos -= 3;
                                        if(vstring[pos - 1] != '-')
                        ...
                        sprintf(vstring, "%.*lG", MAXPLACES, value);
        }
} /* valuestring */

char *cellstring(int col, int row, int formatting)
/* Creates an output string for the data in the cell in (col, row) */
{       CELLPTR cellptr = cell[col][row];
        ...
        {       formatvalue = format[col][row];
                ...
                {       case TEXT:
                        ...
                        case VALUE:
                                ...
                                valuestring(cellptr, value, s, col, formatvalue, formatting);
                                break;
                } /* switch */
        }
        return(s);
...
/* If the spreadsheet has been changed, will ask the user if they want to
   save it. */
{       int save;
        if(changed && getyesno(&save, MSGSAVESHEET) && (save == YESCHAR))
                savesheet();
} /* checkforsave */
